...
public class BatchConfiguration {
	...
	@Bean("step1")
	public Step step1() {
		return stepBuilderFactory.get("step1").<Person, Person>chunk(3).reader(reader()).processor(processor())
				.writer(writer(dataSource())).build();
	}

	@Bean
	public Job importPersonJob(JobCompletionNotificationListener listener, @Qualifier("step1") Step step1) {
